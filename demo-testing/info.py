"""
compare_pre_post.py  —  AI Code Detection: Pre vs Post LLM Era
────────────────────────────────────────────────────────────────
Finds paired *-prellm_predictions.csv / *-postllm_predictions.csv,
writes a clean report + 5 paper-worthy visuals.

Usage:
    python compare_pre_post.py [SEARCH_DIR]
    # If SEARCH_DIR omitted, uses <repo>/results

Output: <search_dir>/<DOMAIN>_summary_results/
    ├── report.txt
...
def process_domain(domain, pre_path, post_path, search_dir):
    ...
    if not pre_rows or not post_rows:
        print("  !! One or both CSVs empty — skipped."); return
...
def main():
    ...
    search_dir = Path(sys.argv[1]) if len(sys.argv) > 1 else (_repo_root / "results")
    pairs = find_pairs(search_dir)
    if not pairs:
        print(f"No paired pre/post CSVs found under: {search_dir}"); return
    print(f"\nFound {len(pairs)} domain pair(s):")
    for d, p, q in pairs:
        print(f"  {d}  |  {p.name}  +  {q.name}")
